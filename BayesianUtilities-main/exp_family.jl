...
#--------------------------
#we use variant 2 in https://en.wikipedia.org/wiki/Exponential_family
function exp_family(p::Beta)
    h(x::Number) = 1
    T(x::Number) = [log(x),log(1-x)]
    η = [p.α - 1, p.β - 1]
    A_eval = loggamma(p.α) + loggamma(p.β) - loggamma(p.α + p.β)
    A(η::Array) = loggamma(η[1]+1) + loggamma(η[2]+1) - loggamma(sum(η)+2)
...
function exp_family(t::Type{F}, η::Vector) where F<:Beta
    α, β = η[1] + 1, η[2] + 1
    Beta(α, β)
end
...
#--------------------------
function exp_family(p::Bernoulli)
    h(x::Number) = 1
    T(x::Number) = x
    η = log(mean(p)/(1-mean(p)))
    A_eval = -log(1-mean(p))
    A(η::Number) = log(1+exp(η))
...
function exp_family(t::Type{F}, η::Number) where F<:Bernoulli
    p = exp(η)/(1+exp(η))
    Bernoulli(p)
end
