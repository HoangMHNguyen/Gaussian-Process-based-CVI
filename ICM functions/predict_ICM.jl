...
# This file stores the prediction function of ICM for the future value
function icm_predict(x_obser::Matrix, y_obser::Matrix, x_new::Vector, A::Matrix, σ::Real, l::Real)
    ...
    K_new = autocov_mat(x_new,A,σ,l); #auto-covariance of new points
...
function icm_predict(x_obser::Matrix, y_obser::Matrix, x_new::Vector, A::Matrix, σ::Real, l::Vector)
    ...
    Kff = autocov_mat(x_obser,A,σ,l); #auto-covariance of training points
    K_new = autocov_mat(x_new,A,σ,l); #auto-covariance of new points
    Kff += 1e-5*Matrix(I, size(Kff,1),size(Kff,1)); #add noise for stable computation
    K_new += 1e-5*Matrix(I, size(K_new,1),size(K_new,1)); #add noise for stable computation
...
function icm_predict(x_obser::Vector, y_obser::Vector, x_new::Vector, A::Matrix, σ::Real, l::Vector)
    Kff = autocov_mat(x_obser,A,σ,l); #auto-covariance of training points
    K_new = autocov_mat(x_new,A,σ,l); #auto-covariance of new points
    Kff += 1e-5*Matrix(I, size(Kff,1),size(Kff,1)); #add noise for stable computation
    K_new += 1e-5*Matrix(I, size(K_new,1),size(K_new,1)); #add noise for stable computation
